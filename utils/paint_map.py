...
# --- Existing Functions ---
def generate_paint_map(image_array, target_rgb, threshold=8.0, overlay_color=[255, 0, 0], overlay_alpha=0.6, dim_factor=0.2):
...
# --- New Function ---
def generate_layered_steps(image_array, palette, threshold=8.0, dim_factor=0.2):
    gray = np.dot(image_array[..., :3], [0.2989, 0.5870, 0.1140])
    base_canvas = np.stack((gray,)*3, axis=-1) * dim_factor
    ...
    for rgb_color in palette:
        ...
        delta_e = delta_e_76(img_lab, target_lab)
        mask = delta_e < threshold
        current_canvas[mask] = rgb_color
        steps.append(Image.fromarray(current_canvas.copy()))
